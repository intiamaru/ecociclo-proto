...
def main():
    ...
    #   if "1" in dataset:
    #    print(json.dumps(dataset["1"], sort_keys=True, indent=4))

    res = get_vendor_summary_text(filter)
    #for c in res:
    #    print('{}:{}'.format(c,ascii(c)))
    print(res)
...
def get_vendor_summary_html(filter):
    result = ""
    text = get_vendor_summary_text(filter)
    result = textile.textile(text)
    return result

def get_vendor_summary_text(filter):
    result = ""
    df = get_summary(tag=filter)
    df = df[["vendor","title","quantity"]]
    gk = df.groupby(['vendor','title']).sum()
#    gk.first()
#    for key, item in gk:
#        vendor = gk.get_group(key)
#        vendor = vendor[["title", "quantity"]]
#        result += " ".join([key,"\n\n", vendor.to_string(index=False), "\n\n"])
    return gk.to_string()
